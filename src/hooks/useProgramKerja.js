...
const API_URL = `${import.meta.env.VITE_API_BASE_URL}/api/v1/proker/`;
...
const getApiService = (authHeader) => ({
    ...
    delete: async (id) => {
        ...
        if (!response.ok && response.status !== 204) {
             throw new Error('Failed to delete program kerja');
        }
    },
});

export const useProgramKerja = (initialSearchQuery = '') => {
    ...
    const fetchData = useCallback(async (url) => {
        ...
        try {
            ...
            setError('Could not load program kerja. Please try again later.');
        } finally {
    ...
    const saveProgramKerja = useCallback(async (data, currentItem) => {
