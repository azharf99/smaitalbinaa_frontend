...
const API_URL = `${import.meta.env.VITE_API_BASE_URL}/api/v1/student-project-teams/`;
...
const getApiService = (authHeader) => ({
    ...
    delete: async (id) => {
        ...
        if (!response.ok && response.status !== 204) {
             throw new Error('Failed to delete student team');
        }
    },
});

export const useStudentTeams = (initialSearchQuery = '') => {
    ...
    const fetchData = useCallback(async (url) => {
        ...
        try {
            ...
            setError('Could not load student teams. Please try again later.');
        } finally {
    ...
    const saveStudentTeam = useCallback(async (data, currentItem) => {
    ...
    const deleteStudentTeam = useCallback(async (id) => {
        try {
