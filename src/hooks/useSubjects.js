...
const API_URL = `${import.meta.env.VITE_API_BASE_URL}/api/v1/subjects/`;
...
export const useSubjects = (initialSearchQuery = '') => {
    ...
    const saveSubject = useCallback(async (formData, currentItem) => {
    ...
    const importSubjects = useCallback(async (file) => {
        setIsSubmitting(true);
    ...
    return { items, count, nextPage, previousPage, isDataLoading, isSubmitting, error, searchQuery, setSearchQuery, saveSubject, deleteSubject, handlePageChange, importSubjects };
};