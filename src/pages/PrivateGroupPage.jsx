    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <AsyncSelect cacheOptions defaultOptions  loadOptions={debouncedLoadSubjects} value={formData.pelajaran} onChange={val => handleSelectChange('pelajaran', val)} placeholder="Pilih Pelajaran" className="react-select-container" classNamePrefix="react-select" styles={customSelectStyles} />
            ...
            <div>
                <label htmlFor="santri" className="block text-sm font-medium dark:text-white">Santri</label>
                <AsyncSelect isMulti cacheOptions defaultOptions loadOptions={debouncedLoadStudents} value={formData.santri} onChange={val => handleSelectChange('santri', val)} placeholder="Pilih Santri" className="react-select-container" classNamePrefix="react-select" styles={customSelectStyles} />
            </div>
                            ...
                            {loading ? (
                                ...
                                <tr key={group.id} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                        ...
                                        {(group.santri || []).map((element, index) => (
                                            <p key={index}>{element.student_name}</p>
                                        ))}
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                        <button onClick={() => handleEdit(group)} className="text-indigo-600 hover:text-indigo-900 dark:bg-gray-200 dark:p-1 dark:rounded-sm">Edit</button>
                                        <button onClick={() => handleDelete(group.id)} className="text-red-600 hover:text-red-900 dark:bg-gray-200 dark:p-1 dark:rounded-sm">Delete</button>
                                    </td>
                                </tr>
                            )) : (
                                <tr>
