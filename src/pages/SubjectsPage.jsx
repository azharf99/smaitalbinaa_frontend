...
const SubjectsTable = ({ items, onEdit, onDelete, error, hasSearchQuery }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md overflow-x-auto">
            <h2 className="text-2xl font-bold mb-4 text-gray-800">Subjects List</h2>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {items.length > 0 ? items.map(item => (
                        <tr key={item.id}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.short_name}</td>
                        ...
                        <tr><td colSpan={columns} className="px-6 py-4 text-center text-sm text-gray-500">{hasSearchQuery ? 'No subjects match your search.' : 'No subjects found.'}</td></tr>
                    )}
                </tbody>
            </table>
        </div>
    ...
    return (
        <>
            <header className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight">Subject Management</h1>
                    <p className="mt-2 text-lg text-gray-600">Manage academic subjects.</p>
                </div>
                {isAuthenticated && (
