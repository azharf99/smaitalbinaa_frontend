...
import { useProgramKerja } from '../hooks/useProgramKerja.js';
...
const ProgramKerjaForm = ({ currentItem, onSave, onCancel, isSubmitting }) => {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" id="waktu_pelaksanaan" name="waktu_pelaksanaan" placeholder="e.g., Januari 2024 - Maret 2024" value={formData.waktu_pelaksanaan || ''} onChange={handleChange} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200 text-gray-900 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400" disabled={isSubmitting} />
                </div>
                <div>
                    <label htmlFor="anggaran" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Anggaran</label>
                    <input type="text" id="anggaran" name="anggaran" placeholder="Anggaran" value={formData.anggaran || ''} onChange={handleChange} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200 text-gray-900 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400" disabled={isSubmitting} />
                </div>
                <div>
                    <label htmlFor="penanggungjawab" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Penanggung Jawab</label>
                    <input type="text" id="penanggungjawab" name="penanggungjawab" placeholder="Penanggung Jawab" value={formData.penanggungjawab || ''} onChange={handleChange} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200 text-gray-900 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400" disabled={isSubmitting} />
                </div>
            </div>
            <div>
...
const ProgramKerjaTable = ({ items, onEdit, onDelete, error, hasSearchQuery }) => {
...
const LoadingProgramKerjaTable = () => {
    const { isAuthenticated } = useAuth();
...
export default function ProgramKerjaPage() {
    ...
    const handleSave = async (formData) => {
        try {
            await saveProgramKerja(formData, currentItem);
            closeModal();
        } catch (err) {
            console.error("Failed to save program kerja:", err);
            alert(`Failed to save program kerja. Error: ${err.message}`);
        }
    };
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this program kerja?')) {
    ...
    return (
        <>
            <header className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight dark:text-white">Program Kerja Management</h1>
                    <p className="mt-2 text-lg text-gray-600 dark:text-white">Manage program kerja information.</p>
                </div>
                {isAuthenticated && (
            ...
            <Modal isOpen={isModalOpen} onClose={closeModal} title={currentItem && currentItem.id ? 'Edit Program Kerja' : 'Add New Program Kerja'}>
                <ProgramKerjaForm 
