...
const ReportForm = ({ currentItem, onSave, onCancel, isSubmitting }) => {
    ...
    const debouncedLoadExtracurriculars = debounce((s, c) => loadOptions(EXTRACURRICULARS_API_URL, s, i => ({ value: i.id, label: i.name })).then(c), 300);
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden transform hover:scale-105 transition-transform duration-300">
            ...
            <div className="p-4">
                ...
                <div className="mt-4 flex justify-end space-x-2">
                    <button onClick={() => onEdit(report)} className="text-sm text-indigo-600 hover:text-indigo-900 font-medium">Edit</button>
                    <button onClick={() => onDelete(report.id)} className="text-sm text-red-600 hover:text-red-900 font-medium">Delete</button>
                </div>
            </div>
        </div>
    );
